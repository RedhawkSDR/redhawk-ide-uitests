...
public class UsesDeviceTest extends AbstractGraphitiTest {
	...
	public void usesDevice_frontEndTuner_controlMode_simRxDigitizer() throws IOException {
		...
		for (SimpleRef simpleRef : structRef.getSimpleRef()) {
			Assert.assertTrue("FRONTEND::tuner_allocation has unexpected simple " + simpleRef.getRefID(), idToValue.containsKey(simpleRef.getRefID()));
			Assert.assertEquals("Unexpected value for field " + simpleRef.getRefID(), idToValue.remove(simpleRef.getRefID()), simpleRef.getValue());
		}
	...
	/**
	 * IDE-2178 Allow scanner allocation via FEI allocation wizard
	 * @throws IOException
	 */
	@Test
	public void usesDevice_scanner() throws IOException {
		waveformName = "usesDevice_scanner";
		...
		// Add a usesdevice for a FrontEnd scanner in control mode
		String usesDeviceId = "scanner_1"; // auto generated by wizard
		String tunerType = "RX_SCANNER_DIGITIZER";
		String newAllocationId = "abc";
		String centerFrequency = "1";
		String bandwidth = null;
		String sampleRate = null;
		boolean deviceControl = true;
		String groupID = "group1";
		String rfFlowID = "flow1";
		String minFreq = "1";
		String maxFreq = "2";
		String mode = "SPAN_SCAN";
		String controlMode = "TIME_BASED";
		String controlLimit = "3";

		DiagramTestUtils.addUseFrontEndTunerDeviceToDiagram(gefBot, editor, 0, 0);

		FETunerControl tunerControl = new FETunerControl(tunerType, newAllocationId, centerFrequency, bandwidth, sampleRate, deviceControl, rfFlowID, groupID);
		UsesDeviceTestUtils.completeUsesFEDeviceWizard(bot, "test.scanner", tunerControl, null, null);
		SWTBotShell tunerShell = bot.shell("Allocate Tuner");
		SWTBot shellBot = tunerShell.bot();

		shellBot.textWithLabel("Minimum Frequency (MHz)").typeText(minFreq);
		shellBot.textWithLabel("Maximum Frequency (MHz)").typeText(maxFreq);
		shellBot.comboBoxWithLabel("Mode").setSelection(mode);
		shellBot.comboBoxWithLabel("Control Mode").setSelection(controlMode);
		shellBot.textWithLabel("Control Limit").typeText(controlLimit);
		shellBot.button("&Finish").click();

		bot.waitUntil(Conditions.shellCloses(tunerShell));
		editor.setFocus();
		...
		idToValue.put("FRONTEND::tuner_allocation::tuner_type", tunerType);
		idToValue.put("FRONTEND::tuner_allocation::allocation_id", newAllocationId);
		idToValue.put("FRONTEND::tuner_allocation::center_frequency", "1000000.0");
		idToValue.put("FRONTEND::tuner_allocation::bandwidth", "0.0");
		idToValue.put("FRONTEND::tuner_allocation::bandwidth_tolerance", "20.0");
		idToValue.put("FRONTEND::tuner_allocation::sample_rate", "0.0");
		idToValue.put("FRONTEND::tuner_allocation::sample_rate_tolerance", "20.0");
		idToValue.put("FRONTEND::tuner_allocation::device_control", "true");
		idToValue.put("FRONTEND::tuner_allocation::group_id", groupID);
		idToValue.put("FRONTEND::tuner_allocation::rf_flow_id", rfFlowID);
		...
		structRef = ud.getStructRef().get(1);
		Assert.assertEquals("FRONTEND::scanner_allocation", structRef.getRefID());
		idToValue.put("FRONTEND::scanner_allocation::min_freq", "1000000.0");
		idToValue.put("FRONTEND::scanner_allocation::max_freq", "2000000.0");
		idToValue.put("FRONTEND::scanner_allocation::mode", mode);
		idToValue.put("FRONTEND::scanner_allocation::control_mode", controlMode);
		idToValue.put("FRONTEND::scanner_allocation::control_limit", "3.0");
		for (SimpleRef simpleRef : structRef.getSimpleRef()) {
			Assert.assertTrue("FRONTEND::scanner_allocation has unexpected simple " + simpleRef.getRefID(), idToValue.containsKey(simpleRef.getRefID()));
			Assert.assertEquals("Unexpected value for field " + simpleRef.getRefID(), idToValue.remove(simpleRef.getRefID()), simpleRef.getValue());
		}
		Assert.assertEquals(0, idToValue.size());
		...
		editor.getEditPart(SadTestUtils.USE_FRONTEND_TUNER_DEVICE).select();
		editor.clickContextMenu("Edit Use FrontEnd Tuner Device");
		tunerShell = bot.shell("Allocate Tuner");
		shellBot = tunerShell.bot();
		...
		Assert.assertEquals(tunerType, shellBot.comboBoxWithLabel("Tuner Type").getText());
		Assert.assertEquals(centerFrequency, shellBot.textWithLabel("Center Frequency (MHz)").getText());
		Assert.assertTrue(shellBot.checkBox("Any Value", 0).isChecked());
		Assert.assertTrue(shellBot.checkBox("Any Value", 1).isChecked());
		Assert.assertEquals("20", shellBot.textWithLabel("Bandwidth Tolerance (%)").getText());
		Assert.assertEquals("20", shellBot.textWithLabel("Sample Rate Tolerance (%)").getText());
		Assert.assertEquals(rfFlowID, shellBot.textWithLabel("RF Flow ID").getText());
		Assert.assertEquals(groupID, shellBot.textWithLabel("Group ID").getText());
		...
		Assert.assertEquals(minFreq, shellBot.textWithLabel("Minimum Frequency (MHz)").getText());
		Assert.assertEquals(maxFreq, shellBot.textWithLabel("Maximum Frequency (MHz)").getText());
		Assert.assertEquals(mode, shellBot.comboBoxWithLabel("Mode").getText());
		Assert.assertEquals(controlMode, shellBot.comboBoxWithLabel("Control Mode").getText());
		Assert.assertEquals(controlLimit, shellBot.textWithLabel("Control Limit").getText());
		shellBot.button("Cancel").click();
