		...
		for (final String componentId : components) {
			...
			editorBot.waitUntil(new DefaultCondition() {
				...
				public boolean test() throws Exception {
					Assert.assertEquals(componentId + SUFFIX, ci.getId());
					return true;
				}

				@Override
				public String getFailureMessage() {
					return "Model component instantiation ID does not match Components tab ID field";
				}
			});

			editorBot.textWithLabel("Component ID:").selectAll().setText("");
			EditorUtils.assertEditorTabInvalid(editor, EditorUtils.SAD_EDITOR_COMPONENTS_TAB_ID);

			editorBot.textWithLabel("Component ID:").selectAll().setText(componentId);
